...
static void allLedsToggle(void)
{
    BSP_LED_Toggle(LED3);
    BSP_LED_Toggle(LED6);
    BSP_LED_Toggle(LED7);
...
static void ledsCircle(void)
{
	static uint8_t led_no = 0;

	for(uint8_t a = 0; a < 8; a++)
	{
		if(a == led_no) BSP_LED_On((Led_TypeDef)a);
		else BSP_LED_Off((Led_TypeDef)a);
	}

	if(led_no >= 7) led_no = 0;
	else led_no ++;
}
...
int main(void)
{
  ...
  //CDC_Transmit_FS("HELLO WORLD!", 13);

  while (1)
  {
	int16_t acc_buffer[3] = { 0 };
	float gyro_buffer[3];

	BSP_ACCELERO_GetXYZ(acc_buffer);
	BSP_GYRO_GetXYZ(gyro_buffer);

	MessageLength = sprintf(
			DataToSend,
			"ACC: X: %d, Y: %d, Z: %d, GYRO: X: %d, Y: %d, Z: %d\r\n",
			(int) acc_buffer[0], (int) acc_buffer[1], (int) acc_buffer[2],
			(int) gyro_buffer[0], (int) gyro_buffer[1],	(int) gyro_buffer[2]
	);
